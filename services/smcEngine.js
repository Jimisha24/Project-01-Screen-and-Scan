...
function evaluateTimeframeInstitutional(ohlcRaw, timeframe, proximityPct = 10) {
  try {
    ...
    for (let i = startIdx; i < ohlc.length - 2; i++) {
      ...
      if (isC1Bearish && hasFVG) {
        ...
        if (isNearOB) {
          let firstRedTouchStr = "None";
          let firstRedHigh = null;
          let firstRedLow = null;
          let isFirstRedLatest = false;

          for (let j = i + 3; j < ohlc.length; j++) {
            const candle = ohlc[j];
            const touchesOB = candle.low <= obTop;

            if (touchesOB) {
              firstRedHigh = candle.high;
              firstRedLow = candle.low;
              firstRedTouchStr = `$${candle.low.toFixed(2)} - $${candle.high.toFixed(2)}`;
              
              if (j === ohlc.length - 1) {
                isFirstRedLatest = true;
              }
              break;
            }
          }
...
function evaluateAllTimeframesInstitutional(dailyRaw, fourHourRaw, weeklyRaw, monthlyRaw, threeMonthRaw, proximityPct = 10) {
  const matches = [];

  try {
    // 1. Process 4H timeframe safely
    const clean4H = safeParseOHLC(fourHourRaw);
    if (clean4H.length >= 4) {
      const match4H = evaluateTimeframeInstitutional(clean4H, '4H', proximityPct);
      if (match4H) matches.push(match4H);
    }

    // 2. Process Daily and higher timeframes safely
    const cleanDaily = safeParseOHLC(dailyRaw);
    if (cleanDaily.length >= 4) {
      const matchDaily = evaluateTimeframeInstitutional(cleanDaily, 'Daily', proximityPct);
      ...
      const matchWeekly = evaluateTimeframeInstitutional(wData, 'Weekly', proximityPct);
      ...
      const matchMonthly = evaluateTimeframeInstitutional(mData, 'Monthly', proximityPct);
      ...
      const match3M = evaluateTimeframeInstitutional(qData, '3M', proximityPct);
